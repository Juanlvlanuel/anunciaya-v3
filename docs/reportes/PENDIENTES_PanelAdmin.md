...
- [ ] 🔴 **Paso 10 — SQL (pendiente)** — ejecutar `DROP COLUMN` de esas 2 columnas en BD (`docs/migraciones/paso10-drop-region-id-viejas.sql`; el `DROP` cascadea índice+FK). El código ya no las usa. Hacer DEV→prod, snapshot antes. Tras correrlo: cambiar los comentarios "se elimina (Paso 10)" del schema a pasado.
