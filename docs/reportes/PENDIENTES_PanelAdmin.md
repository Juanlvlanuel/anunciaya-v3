...
- [x] 🟢 **Página de cuenta/perfil del usuario** (`/perfil`) — **la sección Membresía/Pagos está CONSTRUIDA y con QA E2E cerrado (28 jun 2026)**: el usuario ve su SUSCRIPCIÓN (estado, próximo cobro/gracia, recibos descargables), recupera tarjeta vía Customer Portal, hace pago manual con comprobante y cambia su método de cobro, todo desde Modo Personal. Detalle: [`docs/arquitectura/Mi_Perfil.md`](../arquitectura/Mi_Perfil.md). **Sigue pendiente** dentro de esa página: los tabs **Datos Personales** y **Seguridad** (contraseña, 2FA, teléfono, dirección, ciudad, avatar) están en placeholder; capturar ciudad/región aquí desbloquearía delegar moderación de usuarios a gerentes por región.
  - [x] 🟢 **Panel de pagos de la membresía con métodos manuales** (CONSTRUIDO 28 jun 2026) — dentro de *Mi Perfil – Modo Personal*. Además de **tarjeta (Stripe)**, ofrece **depósito a tarjeta de AnunciaYA / OXXO** y **transferencia/CLABE**: el dueño ve los datos de cobro, elige meses, **adjunta el comprobante** (R2) y un **admin lo verifica** en la cola **"Por verificar"** del Panel antes de extender la membresía. Los manuales **NO usan Customer Portal**. Reusa `marcarPagado` + subida R2 + `metodo_cobro='manual'`. Datos de cobro = config `datos_cobro_manual` editable en el Panel. Tabla real: **`pagos_manuales_solicitudes`** (migración `2026-06-27-...sql`, corrida en prod). El efectivo lo sigue registrando el vendedor/admin. Ver `docs/arquitectura/Mi_Perfil.md`.
    - [x] 🟢 **Recuperación de morosos con TARJETA — botón "Actualizar tarjeta y reintentar pago"** (CONSTRUIDO 28 jun 2026). En Mi Perfil, para negocios con `metodo_cobro='tarjeta'` en gracia/suspendidos: el botón abre el **Customer Portal de Stripe** (`billingPortal.sessions.create`) donde el dueño **actualiza su tarjeta y paga la factura pendiente** → el webhook `invoice.payment_succeeded` lo regresa a `al_corriente`. Resuelve B2 (recuperación) y Z3. El Portal se usa **solo para tarjeta**; los manuales siguen con comprobante. Ver `docs/arquitectura/Mi_Perfil.md`.
