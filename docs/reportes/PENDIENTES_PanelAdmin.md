...
- [x] ✅ **Sección Suscripciones — CERRADA (módulo completo, 23 jun 2026).** El módulo *es* la **Bitácora financiera** (`eventos_pago`): cobros Stripe + pagos manuales + cancelaciones, solo lectura, con KPIs, filtros, **deep-link a Negocios** y alcance por rol; unifica lo de **Stripe** + `admin_auditoria` y el `event.id` sirve de guard de idempotencia. **Sin V2** (decisión Juan): precio editable/cobro día-1/comisión al cobro/recibos = **hechos** en el Sprint de Stripe; promos/cupones, CSV, reembolsos-en-bitácora y dedup-Redis = **descartados**; la **visibilidad de la membresía para el dueño** vive en `apps/web` (`docs/arquitectura/Mi_Perfil.md`, modo Personal), **no** en el Panel. Verdad: `Panel_Admin/Suscripciones.md` + `_Pendientes.md` §Cierre de alcance.
...
- [x] ✅ **Cancelar transaccional + historial de pagos paginado** (HECHO 10 jun 2026) — `cancelarNegocio` envuelve degradar-dueño + archivar-negocio en una `db.transaction` (Stripe y reversión de vouchers quedan fuera, por externos/idempotentes), evitando el estado parcial; verificado con `probar-acciones-parada2.ts`. `listarPagosNegocio` acepta `?limite=N`; la ficha muestra los últimos 5 + "Ver todos" (pide N+1 y re-consulta sin límite). Harness `probar-pagos-limite.ts`.
- [x] ✅ **Vendedor registra pagos en efectivo de su cartera (manuales)** (HECHO 10 jun 2026) — "Registrar pago" abierto al vendedor solo en SUS negocios de método **manual** (altas Y renovaciones), **sin tarjeta** (los cobra Stripe → 403) y **sin cortesía** (→ 403). Guards en `marcarPagado` + alcance de cartera en `cargarNegocioConAlcance` (`resolverEmbajadorId`, exportado). Ruta `marcar-pagado` ahora 3 roles; UI: footer de la ficha con solo "Registrar pago" para el vendedor + diálogo sin cortesía. Harness `probar-vendedor-registra-pago.ts`. (El control del efectivo —"efectivo por entregar" + corte de caja— sigue en el módulo Vendedores.)
- [x] ✅ **Negocios — verificación a fondo (§4) + 2 riesgos de Stripe descartados** (HECHO 10 jun 2026) — acciones de tarjeta (marcar pagado / pausar / reactivar / cancelar) probadas contra **Stripe real** (`probar-acciones-parada2.ts`); alcance del gerente por matriz verificado (`probar-conteo-negocios.ts` + 403 de otra región en `probar-editar-pago.ts`). Descartados con datos reales: (1) **fecha-vs-webhook** — `marcarPagado` empuja `trial_end=X` y Stripe deja `current_period_end=X` (lo que el webhook reescribe), así la fecha manual sobrevive (`probar-fecha-vs-webhook.ts`); (2) **consistencia Cancelar↔webhook** — en cualquier orden el negocio queda archivado + oculto + cancelado (el webhook tardío no halla al usuario; el de carrera solo añade `es_borrador=true` sin contradecir; `probar-cancelar-vs-webhook.ts`). **Módulo Negocios verificado de punta a punta.**
...
- [ ] 🟡 **Página de cuenta/perfil del usuario** (`/perfil` o `/configuracion`) — no existe. Página dedicada **accesible desde el modo Personal**, donde el usuario gestiona: cambiar contraseña, activar **2FA**, teléfono, dirección, género, ciudad, avatar **y su SUSCRIPCIÓN** (estado "activo hasta X" + botón **reactivar/gestionar pago** vía Customer Portal de Stripe). **Debe vivir en Personal** porque al vencer la cuenta baja a personal y el dueño tiene que poder ver y reactivar su pago **sin** depender del Business Studio (comercial). Además, capturar ciudad/región aquí desbloquea delegar la gestión de usuarios a gerentes por región (hoy suspender usuarios = solo SuperAdmin porque `usuarios` no tiene región). **Alcance de la sección Membresía/Pagos consolidado en [`docs/arquitectura/Mi_Perfil.md`](../arquitectura/Mi_Perfil.md)** (vista de membresía + Z3 recuperar tarjeta/B2 + Z4 pago manual con comprobante), trasladado del cierre de la ronda de Stripe (23 jun).
  - [ ] 🟡 **Panel de pagos de la membresía con métodos manuales** (decisión 22 jun 2026) — dentro de esta página de *Mi Perfil – Modo Personal*. Además de **tarjeta (Stripe)**, ofrecer **depósito a una tarjeta de AnunciaYA** y **transferencia a cuenta bancaria**: el comerciante ve los datos de cobro, **adjunta el comprobante** (o lo envía por **WhatsApp**) y un **admin lo verifica** (cola de verificación en el Panel Admin) antes de extender la membresía. Los métodos manuales **NO usan Customer Portal** (OXXO/SPEI de Stripe descartados: OXXO no admite suscripción recurrente). Reusa `registrarPagoManual` + subida R2 + `metodo_cobro='manual'` + cron de vencimientos. Datos de cobro (tarjeta destino / CLABE / WhatsApp) = config nueva en el Panel. Piezas: config de datos de cobro · tabla `solicitudes_pago_membresia` · UI en Mi Perfil · cola de verificación en Admin · notificaciones. El efectivo lo sigue registrando el vendedor/admin. Ver `docs/arquitectura/Panel_Admin/Ronda_Pruebas_Pagos.md` §Z4.
    - [ ] 🟡 **Recuperación de morosos con TARJETA — botón "Actualizar tarjeta y reintentar pago"** (decisión 22 jun 2026). En el mismo Panel de pagos de Mi Perfil, para negocios con `metodo_cobro='tarjeta'` en gracia/suspendidos: botón que abre el **Customer Portal de Stripe** (`billingPortal.sessions.create`) donde el dueño **actualiza su tarjeta y paga la factura pendiente** → el webhook `invoice.payment_succeeded` lo regresa a `al_corriente`. Hoy un moroso de tarjeta **no puede recuperarse solo** (depende del reintento de Stripe o del admin). Resuelve B2 (recuperación) y Z3. El Portal se usa **solo para tarjeta**; los manuales siguen con comprobante. Ver `Ronda_Pruebas_Pagos.md` §B2/§Z3.
