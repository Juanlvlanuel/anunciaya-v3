...
- **`scrollbar-width` (estándar) y `::-webkit-scrollbar` (webkit) son MUTUAMENTE EXCLUYENTES en Chrome/Edge 121+** — Si un elemento declara la propiedad estándar `scrollbar-width` (aunque sea `thin`), Chrome/Edge modernos usan el **renderizado estándar** de la barra (controlado por `scrollbar-width` + `scrollbar-color`) e **IGNORAN por completo** todos los pseudo-elementos `::-webkit-scrollbar-*` de ese elemento. **Síntoma que provocó el bug (carrusel "Coyo encontró esto"):** con `.coyo-scroll { scrollbar-width: thin; scrollbar-color: thumb track }` + `::-webkit-scrollbar-track { background }` + `::-webkit-scrollbar-thumb { background }`, Chrome pintaba **solo la canaleta** (el track venía del `scrollbar-color`, 2º valor) pero el **thumb no se veía** — el thumb custom de `::-webkit-scrollbar-thumb` se descartaba, y el thumb estándar quedaba invisible en esa build. Cambios de color/grosor/border del thumb webkit no tenían NINGÚN efecto porque Chrome no los leía. **Diagnóstico definitivo (sin depender de screenshot):** medir `el.offsetHeight - el.clientHeight` = alto reservado por la barra clásica. Con `scrollbar-width: thin` daba `10px` (barra estándar fina); al quitarlo, respetaba `::-webkit-scrollbar { height: 12px }` → `12px`. Si `getComputedStyle(el).scrollbarWidth === 'thin'`, los `::-webkit-scrollbar` están anulados. **Regla:** para control fino y garantizado del thumb en Chrome/Edge/Safari, usar `::-webkit-scrollbar` **SIN** declarar `scrollbar-width` en ese mismo elemento. Dar `scrollbar-width`/`scrollbar-color` **solo a Firefox** (que no soporta `::-webkit-scrollbar`) dentro de `@supports (-moz-appearance: none) { ... }` — ese `@supports` matchea en Firefox y no en Chrome. Ver `.coyo-scroll` en `apps/web/src/index.css`. **Trampa de diagnóstico:** `preview_screenshot` se colgaba en este proyecto (PWA/service worker), pero las métricas por DOM (`offsetHeight - clientHeight`, `getComputedStyle().scrollbarWidth`) alcanzaron para confirmar causa y fix sin imagen.
...
- **`verificarToken` autentica, NO autoriza — un `:negocioId` en la URL exige guard de propiedad** — Tener sesión válida (`verificarToken`) solo prueba *quién eres*, no *que el recurso sea tuyo*. Si una ruta recibe `:negocioId` (u otro id de recurso) en la URL y solo aplica `verificarToken`, cualquier usuario logueado puede pasar el id de OTRO y modificar/publicar su recurso. **Regla:** toda ruta con un id de recurso en la URL necesita un guard de autorización que compare el dueño del recurso contra el usuario del token (ej. `verificarPropietarioNegocio`, que valida `negocios.usuarioId === req.usuario.usuarioId`). Y la identidad **nunca** se lee del body — `finalizar` tomaba `usuarioId` del body; corregido a `req.usuario.usuarioId`. Descubierto en `onboarding.routes.ts`: el guard existía pero estaba comentado ("Crearemos después") y todas las rutas `:negocioId` quedaban abiertas a negocios ajenos.
- **`router.use('/:param', guard)` hace match de PREFIJO y captura las rutas hermanas de primer nivel — registrar las rutas exentas ANTES del guard** — Un `router.use('/:param', ...)` no se limita a `/:param/algo`: matchea cualquier ruta de un solo segmento, tratándola como valor de `:param`. Verificado empíricamente: con el guard montado primero, `GET /mi-negocio` entra al guard con `param = "mi-negocio"`, no encuentra ese "negocio" y devuelve 404 — rompiendo una ruta que NO debía pasar por el guard. **Regla:** las rutas sin el parámetro (`/mi-negocio`, `/upload-imagen`) deben registrarse **antes** del `router.use('/:param', guard)`; Express evalúa las capas en orden de registro, así que una ruta exacta declarada antes responde primero y el guard solo alcanza a las rutas con el parámetro declaradas después. Caso real en `onboarding.routes.ts`: hubo que mover `/upload-imagen` (estaba intercalada entre rutas `:negocioId`) arriba junto a `/mi-negocio`, y colocar `router.use('/:negocioId', verificarPropietarioNegocio)` justo después de ambas.
