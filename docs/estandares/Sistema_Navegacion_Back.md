...
**Regresión tras feeds nuevos + BS Publicaciones (jul-2026).** Se agregó un feed de publicaciones estilo Facebook a Negocios (calco fiel de MarketPlace) y se rediseñó el de Servicios/MarketPlace; también un módulo "Publicaciones" en Business Studio. Diagnóstico con 4 agentes en paralelo + barrido final anclado en `git log`. Hallazgos y fixes:
- **BS — el back no regresaba a Home tras navegar entre módulos:** el mecanismo central (`esModuloBS` en `useNavegarASeccion.ts`) ya estaba bien diseñado y la mayoría de los componentes de navegación (Menu/Drawer/Swipe/Franja) ya lo usaban — la causa fueron **5 puntos concretos con `navigate()` crudo** que se lo saltaban: `ColumnaIzquierda.tsx` (botón de entrada a BS + widget Clientes/Transacciones, visible en modo comercial fuera de BS) y, sobre todo, **`PaginaPublicaciones.tsx`** (abrir crear/editar el composer vía `?crear=1`/`?editar=<id>`) — cada apertura del composer apilaba una entrada de historial que nunca se limpiaba, por eso "entre más navegas, más lejos queda el Home". Los 5 se migraron a `useNavegarASeccion`, que ya resuelve el `replace` automático (incluso con query string, porque `esModuloBS` compara con `startsWith` y la comparación de igualdad against `location.pathname` sin query basta para detectar el salto).
- **Colisión de `discriminador` en modales anidados de composer (desktop, bug real confirmado):** `ModalAdaptativo.tsx` no exponía prop `discriminador` — el composer (outer) y su diálogo interno "¿Quieres terminar la publicación más tarde?" (inner) caían ambos en el default `_modalUI` de `Modal.tsx`. Como `useBackNativo` solo verifica si la *key* sigue presente en el state (no el *id* de la instancia), un back con ambos abiertos no cerraba nada (la key seguía ahí, solo con el id del otro) y el SIGUIENTE back cerraba los dos de golpe. Fix: `ModalAdaptativo` ahora acepta `discriminador?: string` y lo reenvía a `Modal` (rama escritorio; `ModalBottom` no lo necesitaba aquí — en móvil el composer usa página completa con `useBackNativo` propio, no pasa por `ModalBottom`). Aplicado en los 3 composers (Negocios/MarketPlace/Servicios): el outer reutiliza el mismo `` `_composerXFull_${guardKey}` `` que ya usaba móvil (así también hereda el re-armado tras interceptar el cierre), el inner usa una clave fija propia (`_composerXConfirmarSalir`).
- **Modal de comentarios "con la publicación completa" vs. página de detalle:** confirmado con Juan que son los dos mecanismos que YA existen y ya están enganchados (modal único que junta post+comentarios vía `ModalAdaptativo`, y la página dedicada vía `useVolverAtras`) — no hay un tercer modal anidado pendiente. El componente viejo `ModalArticuloDetalle.tsx` (MarketPlace) que sí anidaba un modal aparte quedó como código muerto (sin imports) desde que se unificó en un solo modal; no se tocó.
- **`PaginaGuardados.tsx` — consistencia:** el tab Servicios navegaba al detalle con `navigate()` crudo mientras el de MarketPlace (mismo archivo) ya usa `navegarASeccion`; iguala el patrón (sin bug activo hoy, `/servicios/:id` no es top-level).
- **Barrido final (deep-links hacia lo nuevo):** notificaciones de comentarios de Negocios, ChatYA (`servicio_publicacion`), buscadores de Negocios/Servicios y Home/Coyo — todos correctamente enganchados o, en el caso de Coyo, no extendidos aún a estas publicaciones (no hay nada roto, solo no implementado todavía).
...
- **`history.replaceState` crudo para limpiar deep-links** (`PaginaNegocios.tsx`, `PaginaMarketplace.tsx`, `PaginaPerfilNegocio.tsx`, `PaginaServicio.tsx`): los 4 bypasean React Router en vez de `navigate(..., {replace:true})` — patrón preexistente replicado fielmente, sin bug confirmado, pero es el anti-patrón que este doc marca como ❌. Sugerido: pasada de QA en Android Chrome sobre el back al entrar por esos 4 deep-links antes de decidir si vale la pena tocarlos los 4 juntos.
- **ChatYA sin tarjeta de contexto para publicaciones libres de Negocio** (el feed nuevo): hoy no se puede iniciar un chat desde ahí, a diferencia de `articulo_negocio`/`oferta_negocio`/`servicio_publicacion`. No es bug de navegación — decisión de alcance de producto pendiente.
