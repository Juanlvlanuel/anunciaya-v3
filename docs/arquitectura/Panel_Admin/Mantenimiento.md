...
- **Tareas programadas** — los **7 crons activos** del backend, cada uno con su **cadencia** (cada 6 h,
...
| `GET` | `/api/admin/mantenimiento/crons` | Catálogo de los 7 crons + telemetría de su última corrida. |
...
- **`cronRegistry.ts`** — catálogo estático de los **7 crons activos** (el de comisiones se retiró, no
  ...
  telemetría y estima la próxima corrida para los de intervalo fijo.
