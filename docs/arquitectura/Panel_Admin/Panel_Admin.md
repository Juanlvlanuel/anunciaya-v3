...
5. **Suscripciones / membresías** — **Tiene documento propio:** 📄 **[`Suscripciones.md`](Suscripciones.md)** (la **bitácora financiera** = el libro mayor de todos los eventos de pago: cobros de Stripe + pagos manuales + cancelaciones, **solo lectura**, con KPIs de ingresos/fallidos y alcance por rol). **Estado: bitácora V1 construida y en uso** (11 Jun 2026; tabla `eventos_pago` + persistencia defensiva en el webhook + lectura, verificada con harness). El **resto del módulo sigue pendiente**: precio de membresía, promos de pago (ej. 3 meses con descuento, pago anual), regalar meses gratis a negocios puntuales, y **tiempos configurables**: periodo de gracia para cobros vencidos y duración del trial (hoy 14 días → editable desde el Panel). Los tiempos viven en `configuracionSistema` (la tabla ya tiene `trial_duracion_dias=14` y otras configs de trials/pagos).
