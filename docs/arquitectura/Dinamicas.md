...
> **Estado:** 🟡 En construcción — Fases 1-3 completas y en producción. Fase 4.1 (sala en vivo + motor de sorteo), **Fase 4.2 (tómbola 3D real + modos automático/manual)** y **Fase 4.3 completa (los 3 métodos de sorteo: `tombola`, `carta_unica`, `tabla_completa`)** construidas, migraciones corridas en DEV/PROD (`tabla_completa` no necesitó ninguna — mismo catálogo estático que `carta_unica`). Sala como **página completa** (`/marketplace/dinamica/:id/sala` y `/p/.../sala`), con el **Cuadro de Honor** ("Últimos Ganadores").
>
> **👉 Próximo paso concreto:** `tombola`/`carta_unica` ya pasaron por varias rondas de QA E2E en vivo (con ajustes de UI/UX incluidos) — falta el pase final en producción real. **`tabla_completa` (commit `e1cadd9`, 10-ago) todavía NO se ha probado ni una sola vez en el navegador** — solo verificado con `tsc -b` + tests unitarios. Es lo primero que hay que probar en vivo al retomar: crear una rifa con ese método → ver el grid de tablas en la ficha → programar sala → sortear (checar "Tu tabla" marcándose en vivo y, si se puede forzar, un empate real) → cerrar y ver ganadores. Después de eso, Fase 5 (tarjeta compartible) sigue 🔜 sin empezar.
...
| **Fase 4.1** | Sala en vivo (chat, moderación, cuenta regresiva) + motor de sorteo (tómbola) determinista/auditable por rondas (K×N) | ✅ Backend y frontend construidos (7-ago), migraciones corridas en DEV/PROD (9-ago), **probada E2E en vivo con varias rondas de ajustes de UI/UX** — pendiente el pase final en el dominio de producción real |
| **Fase 4.2** | Tómbola 3D real (Three.js), modo automático (con pausar/reanudar) y manual, catch-up en vivo para quien se une a mitad del sorteo, historial completo recomputado para salas cerradas | ✅ Construida (9-ago), **probada E2E en vivo** (mismas rondas de ajustes que 4.1) — pendiente el pase final en producción |
| **Fase 4.3 — `carta_unica`** | Lotería mexicana, carta única (mismo motor de tómbola, piel de 54 cartas) | ✅ Construida (9-ago), **probada E2E en vivo con varias rondas de ajustes** (carrusel, animación de revelación, etc.) — pendiente el pase final en producción |
| **Fase 4.3 — `tabla_completa`** | Lotería mexicana, tabla completa (motor propio `ejecutarSorteoTablaCompleta`, catálogo de 150 tablas, empates vía `reglaDesempate`, evento `carta-cantada`) | ✅ Construida (10-ago), sin migraciones nuevas, **verificada solo con `tsc -b` + tests unitarios — todavía NO se ha probado en vivo en el navegador ni una sola vez**, es la única pieza de Fase 4 sin ronda de QA E2E |
