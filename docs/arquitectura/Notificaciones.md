...
> **Sin `actorNombre`** (se renderizan en modo personal sin actor: título + mensaje). **Deep-link:** los `pago_*` navegan a `/perfil?tab=pagos&movId=<id>` y **resaltan** ese movimiento en el historial unos segundos (scroll + ring ámbar, se limpia solo); `membresia_en_gracia` va a `/perfil?tab=pagos` (sin movimiento puntual). Resuelto por `tipo` en `obtenerRutaDestino()`, ANTES del `if (!referenciaTipo) return null`. **`referenciaId` = id del movimiento** para poder resaltarlo: solicitud rechazada (`pago_rechazado`), recibo generado (`pago_aprobado`), pago anulado (`pago_anulado`); `negocioId` en gracia. Helpers en `notificaciones.service.ts`: `notificarPagoRechazado/Aprobado/Anulado` (nuevos) y `notificarMembresiaEnGracia` (ya existía, ahora enganchado en `manejarCobroFallido`).
>
