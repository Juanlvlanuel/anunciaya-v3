-- 2026-08-07: notificaciones — nuevo tipo 'dinamica_pago_confirmado'
-- =============================================================================
--
-- Al participante (con cuenta AnunciaYA) cuando el organizador confirma que
-- ya recibió su pago del boleto — no aplica a participantes manuales "Sin
-- cuenta AY" (no tienen usuario al que notificar dentro de la app).
--
-- IDEMPOTENTE: DROP CONSTRAINT IF EXISTS + ADD con la lista COMPLETA. Parte
-- de los 41 valores dejados por la migración 2026-08-03-notificaciones-dinamicas.sql
-- + 1 nuevo = 42. `referencia_tipo` no cambia (ya incluye 'dinamica').
--
-- ┌─────────────────────────────────────────────────────────────────────────┐
...
ALTER TABLE notificaciones ADD CONSTRAINT notificaciones_tipo_check CHECK (
    (tipo)::text = ANY ((ARRAY[
        ...
        'dinamica_pospuesta',
        'dinamica_resultado',
        -- ── Dinámicas — pago confirmado (agosto 2026) ────────────────────
        'dinamica_pago_confirmado'
