...
- [~] **Panel Admin** — diseño 100% (`Panel_Admin.md`); **Fase 0 completa (dev + prod)** + **frontend shell+login** (`apps/admin`) + `GET /api/admin/yo` + **sección Negocios en prod** (Entrega 1 VER + Parada 1 + Parada 2 + sucursales + filtro de región del superadmin + migración ciudad↔región + **alta manual sin Stripe / Camino B con sus 6 fases**, 10 Jun). En curso: resto de secciones internas (Usuarios, Suscripciones, Vendedores+comisiones, Equipo y accesos) + despliegue. **Va antes de la beta.**
...
- **Fase 1 — Motor:** **Negocios ✅ en prod (Entrega 1 VER + Parada 1 + Parada 2 de acciones + sucursales + filtro de región del superadmin + alta manual sin Stripe / Camino B con sus 6 fases + cron de expiración de manuales + editar correo del dueño)** · Usuarios · Suscripciones · Vendedores+comisiones · Equipo y accesos.
