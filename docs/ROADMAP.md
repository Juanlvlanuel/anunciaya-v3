...
> **Última actualización:** 8 Julio 2026 (go-live: **AWS SES fuera del sandbox** + **humo E2E cerrado en prod** + payout Stripe al banco + **Sentry (error tracking) integrado y validado E2E**; ver `docs/DESPLIEGUE_PRODUCCION.md`)
...
| **Observabilidad — Error tracking (Sentry)** | ✅ 100% | Backend + web/ScanYA + admin; captura solo en prod; alertas por correo; source maps en Vercel; plan gratis. Validado E2E 8 Jul 2026. Ver `docs/DESPLIEGUE_PRODUCCION.md`. |
...
- [x] **Mi Perfil – Datos Personales + Seguridad (Modo Personal)** ✅ (29 Jun 2026 — los 2 tabs restantes de `/perfil`: avatar / nombre / apellidos / teléfono con lada editable / fecha de nacimiento / género / ciudad (correo solo-lectura), cambiar contraseña con validación en vivo + 2FA; el tab "Membresía y Pagos" ahora es condicional —solo con negocio o publicidad vigente—. Pendiente QA E2E a mano. `docs/arquitectura/Mi_Perfil.md`)
- [ ] Beta privada: 50 negocios piloto (después del Panel)
- [ ] Lanzamiento público
- [x] **Stripe modo LIVE** ✅ (1 Jul 2026 — cuenta activada, keys live en Render+Vercel, webhook live, precios $864/$8640 en Live, Customer Portal, validado E2E con cobro real) — checklist en `docs/arquitectura/Pagos_Suscripciones.md §11.1`
...
| Panel Stripe-P2/P3 | **Sprint de Stripe · Piezas 2 y 3.** **Cobro "día 1"** para ventas por vendedor (sub sin trial + empuje a +44d; alta manual con cortesía visible en el modal) + **comisión recurrente "al cobro"** (anti-doble-pago del prepago: anual = 10× una vez; escalón congelado; marcador `comision_devengada_hasta`; foto mensual retirada). 2 harness (Test Clock + datos reales) TODO VERDE. Migración `2026-06-19-comision-al-cobro.sql` aplicada en **dev y prod** (20 jun). **Validado E2E en la Ronda de Pagos A-Z (22-23 jun).** Docs `Sprint_Stripe.md`, `Vendedores_y_comisiones.md`, `Pagos_Suscripciones.md` (v1.6). | 19 Jun 2026 |
| Panel — cierre de módulos | **Métricas · Resumen · Auditoría · Mantenimiento · Publicidad ✅.** Tablero de inicio (cola de pendientes 4 tipos), vista de análisis (recharts), bitácora de auditoría humanizada, centro técnico (Salud/Crons/Logs/Recolector R2) y venta de publicidad por ciudad (wizard Stripe + cron + recibos). | 20-21 Jun 2026 |
| Ronda de Pagos Stripe (A-Z) | **Stripe validado E2E al 100%** — bloques A-H + decisiones Z + 22 OBS cerrados; webhook (firma/idempotencia/reintentos), reembolsos/disputas documentados como flujo manual. Doc `Ronda_Pruebas_Pagos.md`. | 22-23 Jun 2026 |
| Panel Territorios (módulo 13) | **Mapa de la red de ventas ✅** — zonas (GeoJSON jsonb, snapping, no-traslape turf) asignadas a vendedores; "Mi territorio" del vendedor con marcas de prospección (estado + nota). GATE 2 validado 27 jun + pulido UX móvil. Doc `Territorios.md`. | 26-27 Jun 2026 |
| Mi Perfil (Modo Personal) | **`/perfil` completo (3 tabs) ✅** — Datos Personales + Seguridad (contraseña + 2FA) + Membresía/Pagos condicional (Customer Portal, pago manual con comprobante, cambio de método). Doc `Mi_Perfil.md`. | 28-29 Jun 2026 |
| Panel Categorías (módulo 14) | **Catálogo de giros + disponibilidad por ciudad ✅** — CRUD de categorías/subcategorías desde el Panel sin SQL (2 tablas N:M, global por default, auto-poblado por demanda); endpoint público filtra por ciudad activa; 12ª categoría Bienes Raíces; emojis del catálogo eliminados (contract + DROP). Doc `Categorias.md`. | 29-30 Jun 2026 |
| Observabilidad — Sentry (error tracking) | **Sentry en api/web/admin ✅** — captura automática de errores de producción con stack traces legibles + alertas por correo; solo en prod (dev inerte). Backend `sentry.ts` (init + `setupExpressErrorHandler`) + frontends `config/sentry.ts` (tag `superficie` web/scanya) + `@sentry/vite-plugin` (source maps + release en Vercel). Privacidad: scrubbing de PII + `sendDefaultPii:false`. 3 proyectos (org `anunciaya`), plan gratis. Validado E2E en las 4 superficies. Doc: `DESPLIEGUE_PRODUCCION.md`. | 8 Jul 2026 |
...
- [x] ~~**Perfil Personal del usuario** (`/perfil`)~~ ✅ (29 Jun 2026) — construido con sus 3 tabs (`docs/arquitectura/Mi_Perfil.md`): **Datos Personales** (avatar, nombre, apellidos, teléfono con lada editable, fecha de nacimiento, género y **ciudad**), **Seguridad** (cambiar contraseña + 2FA) y **Membresía y Pagos** (condicional). Pendiente QA E2E a mano.
  - **Decisión sobre el correo:** quedó **solo-lectura para todos** (no editable desde el perfil), porque cambiarlo exige flujo de re-verificación que está fuera de alcance de la beta. Las distinciones por rol (gerente/dueño/personal) ya no aplican a este alcance.
  - **Ciudad capturada → pre-requisito de delegación cumplido:** el perfil ahora escribe `usuarios.ciudad_id` (vía el selector de ciudad). De ahí saldría la región para, a futuro, delegar la gestión de usuarios a gerentes por región (hoy suspender/bloquear usuarios sigue siendo solo SuperAdmin).
...
- **Fase 1 — Motor ✅:** Negocios (+ alta manual Camino B) · Usuarios · Suscripciones · **Recibos** · Vendedores y comisiones (A·B·C·D·E + Stripe pieza 3) · Equipo y accesos.
- **Fase 1.5 — Operación ✅:** Resumen · Métricas · Ciudades · **Categorías** · Configuración · Publicidad · Sistema (Auditoría + Mantenimiento).
- **Fase 2 — Vendedores v2 ✅:** mapa de **Territorios** (zonas + marcas). Backlog: cobertura multi-región (Pieza F).
...
- [x] **Sentry (error tracking)** ✅ (8 jul 2026 — api/web/admin; captura solo en prod, alertas por correo, source maps en Vercel, plan gratis; validado E2E en las 4 superficies incl. ScanYA. Ver `docs/DESPLIEGUE_PRODUCCION.md`)
