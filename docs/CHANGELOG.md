...
**"Registrar pago" para el vendedor (cobros en efectivo de su cartera):** el vendedor ya registra pagos en efectivo (altas Y renovaciones) sobre **sus negocios de método manual** desde la ficha — antes solo el primer pago vía el alta. Blindado en el service: solo su cartera (`cargarNegocioConAlcance` + `resolverEmbajadorId`), solo manuales (los de tarjeta → 403, los cobra Stripe) y sin cortesía (→ 403). Ruta `POST /:id/marcar-pagado` ahora `['superadmin','gerente','vendedor']`; el footer de la ficha le muestra solo "Registrar pago" y el diálogo le oculta cortesía. **Pendiente** (módulo Vendedores): el control del efectivo ("efectivo por entregar" + corte de caja + comisión condicionada).

**Verificación (scripts DEV con datos reales):** `probar-alta-manual(-vendedor)`, `probar-alta-tarjeta`, `probar-login-sin-contrasena`, `probar-pagos-negocio`, `probar-existe-correo`, `probar-vencimiento-manual`, `probar-cambiar-correo-dueno`, `probar-editar-pago`, `probar-conteo-negocios`, `probar-pagos-limite`, `probar-fecha-vs-webhook`, `probar-cancelar-vs-webhook`, `probar-vendedor-registra-pago`.
...
`estaFueraDeCirculacion`. **En producción (commit-push 5 Jun). Sin migración.**
...
## [5 Junio 2026] - Panel Admin · Sección Negocios (Entrega 2 — Parada 1: acciones sin Stripe) 🏪⚙️

Las acciones de administración sobre negocios que NO tocan Stripe. La Entrega 2 se dividió en
2 paradas: **Parada 1 (esta)** = suspender, reactivar, reasignar vendedor. **Parada 2 (pendiente)**
= marcar pagado + cancelar (las que sí tocan Stripe, solo SuperAdmin). **En producción
(commit-push 5 Jun).**

**Migración** (`docs/migraciones/2026-06-05-negocios-estado-admin-metodo-cobro-y-auditoria.sql`):
`negocios.metodo_cobro` (tarjeta/manual), `negocios.estado_admin` (activo/suspendido/archivado) +
índice, y tabla nueva `admin_auditoria` para registrar quién hizo qué. Corrida en dev. **Correr
en prod.**

**Acciones (backend, con alcance por rol y auditoría).**
- **Suspender** (SuperAdmin + Gerente de su región): `estado_admin='suspendido'` + `activo=false`
  → oculta el negocio de toda la app (el feed respeta `activo`). Motivo OBLIGATORIO.
- **Reactivar** (reversa de suspender): `estado_admin='activo'` + `activo=true`. Motivo opcional.
- **Reasignar vendedor** (SuperAdmin cualquiera / Gerente de su región): cambia o quita
  `embajadorId` + `regionId`; al quitar mantiene el `regionId`. Motivo opcional.
- **Guard defensivo en el webhook** (`manejarRenovacionPagada`): un pago NO republica un negocio
  suspendido a mano (respeta `estado_admin`). Coherente con la Tanda 1 del frente transversal.

**Frontend (`apps/admin`).** Toast reutilizable del Panel (`stores/useToastPanel.ts` +
`components/ui/Toaster.tsx`), `DialogoConfirmar` (suspender/reactivar) y `DialogoReasignar`. Los
botones se activan en la ficha solo para SuperAdmin/Gerente; los de la Parada 2 (marcar pagado /
cancelar) siguen deshabilitados.

Type-check de `apps/api` y build de `apps/admin` en verde. Probado en dev.
...
(Business Studio, ScanYA). **En producción (commit-push 5 Jun).** Decisión: bloqueo TOTAL del modo comercial
...
verdad de visibilidad**; el MOTIVO vive aparte en `estado_membresia` / `estado_admin`. **En producción
(commit-push 5 Jun).** Reporte: `docs/reportes/REPORTE_Tanda2_Negocio_Fuera_Circulacion.md`.
...
Type-check de `apps/api` y `apps/web` en verde. **En producción (commit-push 5 Jun).** Pendientes anotados: Grupos
