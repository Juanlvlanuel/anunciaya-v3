// PublishWizard.tsx — orquestador del wizard de 3 pasos
import { useEffect, useMemo, useState } from "react";
import {
  Step1, Step2, Step3, Icon,
  WizardData, CategoryId, ModalityId,
} from "./WizardSteps";
...
const DEFAULT: WizardData = {
  ...
  budgetMax: "",
  zones: [],
  confirms: { legal: false, truthful: false, coord: false },
};

const STEP_NAMES = ["Qué necesitas", "Detalles", "Revisa y publica"] as const;
...
/* ------------------------------------------------------------ main */
type Props = {
  /** opcional — callback al publicar (recibe el data validado) */
  onPublish?: (data: WizardData) => void;
};

export function PublishWizard({ onPublish }: Props) {
  ...
  /* mensaje contextual de qué falta */
  const nextHelp = useMemo(() => {
    if (canNext) return "";
    if (step === 0) {
      if (!data.category) return "Selecciona una categoría para continuar.";
      if (data.title.length < 10)
        return `Tu título necesita ${10 - data.title.length} caracteres más.`;
      if (data.description.length < 30)
        return `La descripción necesita ${30 - data.description.length} caracteres más.`;
    }
    if (step === 1) {
    ...
    <div className="app-shell">
      <header className="app-topbar">
        ...
        <span className="breadcrumb">
          Publicar anuncio · <b>{STEP_NAMES[step]}</b>
        </span>
        <div style={{ flex: 1 }} />
        <button
      ...
      {showSuccess && (
        <div className="success-overlay" onClick={() => setShowSuccess(false)}>
          <div className="success-card" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              Ya es visible en el feed de Clasificados. Los vecinos podrán
