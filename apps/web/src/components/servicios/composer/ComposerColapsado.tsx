...
export function ComposerColapsado({
    ...
    // Conteo liviano de publicaciones activas — alimenta el chip
    // "Mis publicaciones · N" del header. Reemplaza al widget lateral
    // `MisPublicacionesWidget` que vivía a la derecha del composer.
    const { data: activasData } = useMisPublicacionesServicio('activa', {
        limit: 1,
        offset: 0,
    });
        ...
        return (
            ...
            >
                ...
                {/* ── Overlay sobre TODO el card ──────────────────────
                    ...
                    "borrador pendiente"). PC: tinte sky casi opaco
                    (`bg-sky-50/95 backdrop-blur-sm`) — NO amber porque
                ...
                <div
                    ...
                    className="absolute inset-0 rounded-2xl border-2 border-amber-400 lg:border-sky-300 bg-amber-50 lg:bg-sky-50/95 lg:backdrop-blur-sm flex items-center lg:flex-col lg:items-center lg:justify-center gap-2 px-3 py-2 lg:px-4 lg:py-3 lg:cursor-pointer hover:bg-amber-100/90 lg:hover:bg-sky-100/95"
                >
                    {/* Móvil: ícono + texto en una línea a la izquierda */}
                    <div className="flex-1 min-w-0 lg:flex-none">
                        <div className="inline-flex items-center gap-1.5 lg:gap-2 text-amber-800 lg:text-sky-800">
                            <FileEdit
                    ...
                    {/* Botones — PC: Descartar blanco neutro + Continuar
                        sky primario destacado (sólidos para que se lean
                        claros sobre el tinte sky del overlay). */}
                    <div className="flex items-center gap-1.5 lg:gap-2 shrink-0 lg:mt-1">
                        ...
                        <button
                            ...
                            className="inline-flex items-center justify-center gap-1.5 h-8 w-8 lg:h-9 lg:w-auto lg:px-4 rounded-lg bg-linear-to-b from-amber-500 to-amber-600 lg:bg-linear-to-b lg:from-sky-500 lg:to-sky-700 text-white text-[13px] font-bold hover:brightness-110 shadow-sm lg:cursor-pointer"
                        >
 ...
 * `MisPublicacionesWidget` que existía en el primer cut del Sprint 9 — la
 * decisión final fue mantener el composer acotado al ancho del composer
 * de MarketPlace y exponer el atajo como botón compacto dentro del header
 * de la pill (mismo patrón que MP para coherencia visual entre secciones).
 *
...
function MisPublicacionesChip({ total }: { total: number }) {
    ...
    return (
        <button
            type="button"
            data-testid="composer-colapsado-mis-pubs"
