...
export function OverlayBuscadorOfertas() {
    ...
    const handleClickSugerencia = (ofertaId: string) => {
        ...
        navigate(`/ofertas?oferta=${ofertaId}`, { replace: true });
    };
    ...
    return (
        <OverlayBuscadorContainer
            ...
            discriminador="_buscadorOfertas"
            ariaLabel="Buscador de Ofertas"
            testId="overlay-buscador-ofertas"
        >
                        ...
                        {cargandoSug && sugerencias.length === 0 ? (
                            ...
                            <ul className="flex flex-col gap-1">
                                {sugerencias.map((oferta, idx) => {
                                    ...
                                    // en negocios). Si es principal → "Matriz",
                                    // si no → su nombre.
                                    const etiquetaSucursal = oferta.totalSucursales > 1
                                        ? (oferta.esPrincipal ? 'Matriz' : oferta.sucursalNombre)
                                        : null;
                                    return (
