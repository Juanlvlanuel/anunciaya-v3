...
export default function ModalRecompensa({
  ...
  const [requiereAprobacion, setRequiereAprobacion] = useState(recompensa?.requiereAprobacion ?? false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white rounded-2xl shadow-2xl border-2 border-slate-300 w-full max-w-md max-h-[90vh] overflow-y-auto">

        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 bg-slate-100 border-b-2 border-slate-300 rounded-t-2xl sticky top-0 z-10">
          ...
          <button onClick={handleCerrar} className="text-slate-400 hover:text-slate-600 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          {/* Requiere aprobación */}
          <div className="flex items-center justify-between py-3 px-4 rounded-xl border-2 border-slate-200 bg-slate-50">
            <div>
              <p className="text-sm font-semibold text-slate-700">Requiere aprobación</p>
              <p className="text-xs text-slate-400 mt-0.5">El canje necesita confirmación manual</p>
            </div>
            <div
              onClick={() => setRequiereAprobacion(!requiereAprobacion)}
              className={`w-10 h-5.5 rounded-full cursor-pointer transition-colors relative ${
                requiereAprobacion ? 'bg-indigo-600' : 'bg-slate-300'
              }`}
            >
              <div
        ...
        {/* Footer */}
        <div className="px-5 py-4 border-t-2 border-slate-200 flex gap-3">
          <button
