...
const NotificacionToast: React.FC<NotificacionToastProps> = ({ notificacion, onClose }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Cerrar — círculo gris discreto, centrado verticalmente en toda la
            burbuja (no solo en la fila del texto) para que no quede pegado
            arriba cuando hay una fila de acción debajo. */}
        <button
          onClick={cerrar}
          aria-label="Cerrar notificación"
          className="absolute right-4 top-1/2 -translate-y-1/2 shrink-0 flex h-7 w-7 items-center justify-center rounded-full bg-slate-200/70 text-slate-600 backdrop-blur-sm transition-colors lg:cursor-pointer lg:hover:bg-slate-300/80"
        >
          <X className="w-4 h-4" strokeWidth={2.5} />
        </button>

        {/* Contenido — fila con items-center: el ícono queda centrado contra
            la altura total de la columna (texto + acción), igual que la X.
            pr ampliado para dejar espacio a la X, que es absoluta. */}
        <div className="flex items-center gap-3 pl-5 pr-14 py-3">
          ...
          <div className="min-w-0 flex-1 flex flex-col gap-2">
            ...
            {/* Acción opcional (ej. "Reintentar") — fila propia, centrada
                horizontalmente en el card. */}
            {notificacion.accion && (
              <div className="flex justify-center">
                <button
                  ...
                  className="shrink-0 rounded-full px-4 py-1.5 text-[13px] font-bold text-white transition-colors lg:cursor-pointer"
                  style={{ background: config.color }}
                >
