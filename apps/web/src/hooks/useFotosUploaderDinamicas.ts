/**
 * useFotosUploaderDinamicas.ts
 * ==============================
 * Hook que encapsula la lógica de subida de fotos del composer de Dinámicas
 * a R2 (presigned URL flow) + tracking de huérfanas.
 *
 * Réplica 1:1 de `useFotosUploaderMarketplace.ts` con las únicas diferencias:
 *   - MAX_FOTOS_COMPOSER_DINAMICA = 12 (mismo máximo que MP/Servicios).
 *   - Usa las mutations de Dinámicas (`useUploadFotoDinamica`,
 *     `useEliminarFotoDinamicaHuerfana`) en vez de las de MarketPlace.
 *   - Las fotos van al bucket con prefijo `dinamicas/` (lo decide el backend
 *     al firmar la presigned URL).
 *
 * Ver `useFotosUploaderMarketplace.ts` para el detalle de por qué el flujo es
 * así (UI optimista con previews locales, subidas paralelas con
 * Promise.allSettled, etc.) — es exactamente el mismo comportamiento.
 *
 * Ubicación: apps/web/src/hooks/useFotosUploaderDinamicas.ts
 */

import axios from 'axios';
import { useEffect, useRef, useState, type MutableRefObject } from 'react';
import {
    useEliminarFotoDinamicaHuerfana,
    useUploadFotoDinamica,
} from './queries/useDinamicas';
...
export const MAX_FOTOS_COMPOSER_DINAMICA = 12;
...
export interface FotoPreviewLocalDinamica {
    tempId: string;
    url: string;
    tipo: 'imagen' | 'video';
}

interface UseFotosUploaderDinamicaOpts {
    fotos: ArchivoFoto[];
...
export function useFotosUploaderDinamicas({
    fotos,
    onCambioFotos,
    urlsSubidasEnSesion,
}: UseFotosUploaderDinamicaOpts) {
    const inputGaleriaRef = useRef<HTMLInputElement>(null);
    const inputCamaraRef = useRef<HTMLInputElement>(null);
    const inputCamaraVideoRef = useRef<HTMLInputElement>(null);
    const [previews, setPreviews] = useState<FotoPreviewLocalDinamica[]>([]);
    const uploadMutation = useUploadFotoDinamica();
    const eliminarHuerfanaMutation = useEliminarFotoDinamicaHuerfana();
    ...
    async function subirImagen(archivo: File): Promise<ArchivoFoto> {
        let blob: Blob;
        try {
            blob = await optimizarImagen(archivo, { maxWidth: 1920, quality: 0.85 });
        } catch {
    ...
    async function manejarArchivos(files: FileList | null) {
        ...
        const espacios = MAX_FOTOS_COMPOSER_DINAMICA - ocupados;
        if (espacios <= 0) {
            notificar.advertencia(`Máximo ${MAX_FOTOS_COMPOSER_DINAMICA} fotos por Dinámica.`);
            return;
        }
        ...
        setPreviews((prev) => prev.filter((p) => !tempIdsProcesados.has(p.tempId)));
        items.forEach((i) => URL.revokeObjectURL(i.url));

        if (exitosas.length > 0) {
            const combinadas = [...fotosRef.current, ...exitosas].slice(0, MAX_FOTOS_COMPOSER_DINAMICA);
    ...
    function puedeAgregar(): boolean {
        ...
        if (ocupados >= MAX_FOTOS_COMPOSER_DINAMICA) {
            notificar.advertencia(`Máximo ${MAX_FOTOS_COMPOSER_DINAMICA} fotos por Dinámica.`);
            return false;
        }
        return true;
    ...
    return {
        ...
        inputGaleriaProps: {
            ...
            multiple: true,
            hidden: true,
            onChange: (e: React.ChangeEvent<HTMLInputElement>) => manejarArchivos(e.target.files),
        },
        inputCamaraProps: {
            ref: inputCamaraRef,
            type: 'file' as const,
