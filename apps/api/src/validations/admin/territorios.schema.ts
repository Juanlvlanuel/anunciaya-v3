...
/** Crear una marca: ubicación + estado (+ nombre, teléfono, nota y ciudad opcionales). */
export const crearMarcaSchema = z.object({
    ...
    telefono: z.string().trim().max(20, 'El teléfono es muy largo.').optional(),
    nota: z.string().trim().max(500, 'La nota es muy larga.').optional(),
    ciudadId: z.string().uuid().nullable().optional(),
});

/** Editar una marca (estado, nombre, teléfono, nota y/o reubicación). */
