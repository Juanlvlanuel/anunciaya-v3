 ...
 *
 * Notificaciones: fuera de alcance en v1 (no se agrega tipo nuevo al enum
 * `TipoNotificacion` compartido ni a su CHECK en BD para esta primera
 * versión — el negocio revisa comentarios desde el feed).
 *
...
export async function listarComentarios(publicacionId: string): Promise<ComentarioNodo[]> {
    const resultado = await db.execute(sql`
        SELECT
            ...
            c.parent_id   AS parent_id,
            c.texto,
            (n.usuario_id = c.autor_id OR u.negocio_id = p.negocio_id) AS es_vendedor,
...
export async function crearComentario(
    ...
    const insertResult = await db.execute(sql`
        INSERT INTO negocio_publicaciones_comentarios (publicacion_id, autor_id, parent_id, texto)
