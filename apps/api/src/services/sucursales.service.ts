	...
	try {
		...
		const resultado = await db.execute(sql`
			SELECT
				...
				s.telefono,
				s.whatsapp,
				s.telefono_alterno AS "telefonoAlterno",
				s.whatsapp_alterno AS "whatsappAlterno",
				s.correo,
				s.activa,
		...
		return resultado.rows.map((row: Record<string, unknown>) => ({
			...
			telefonoAlterno: row.telefonoAlterno as string | null,
			whatsappAlterno: row.whatsappAlterno as string | null,
